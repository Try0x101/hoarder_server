...
async def get_weather_from_wttr(lat: float, lon: float) -> Optional[Dict[str, Any]]:
    try:
        print(f"[{datetime.datetime.now()}] Trying WTTR API for {lat:.3f}, {lon:.3f}")
        result = await fetch_wttr_weather(lat, lon)
        print(f"[{datetime.datetime.now()}] WTTR API success")
        return result
    except Exception as e:
        print(f"[{datetime.datetime.now()}] WTTR API failed: {e}")
        return None

def get_wttr_client_info():
    return {
        'service': 'WTTR',

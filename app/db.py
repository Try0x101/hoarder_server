  ...
  async with pool.acquire() as conn:
   try:
    ...
    if tables_exist<3:
     ...
     await conn.execute("CREATE TABLE IF NOT EXISTS timestamped_data(id SERIAL PRIMARY KEY,device_id TEXT NOT NULL,payload JSONB NOT NULL,data_timestamp TIMESTAMPTZ NOT NULL,received_at TIMESTAMPTZ DEFAULT now(),data_type TEXT DEFAULT 'delta',is_offline BOOLEAN DEFAULT false,batch_id TEXT NULL)")
...
def calculate_delta_changes(current_payload:Dict[str,Any],previous_payload:Dict[str,Any])->Dict[str,Any]:
 ...
 for key,value in current_payload.items():
  if key in context_fields:delta_changes[key]=value
  elif key not in previous_payload or previous_payload[key]!=value:delta_changes[key]=value
...
async def get_timestamped_history(device_id:Optional[str]=None,days:int=30):
 async with pool.acquire() as conn:
  ...
  if device_id:
   rows=await conn.fetch("SELECT device_id,payload,data_timestamp,received_at,data_type,is_offline,batch_id FROM timestamped_data WHERE device_id=$1 AND data_timestamp>=$2 ORDER BY data_timestamp DESC",device_id,time_threshold)
  else:
   rows=await conn.fetch("SELECT device_id,payload,data_timestamp,received_at,data_type,is_offline,batch_id FROM timestamped_data WHERE data_timestamp>=$1 ORDER BY device_id,data_timestamp DESC",time_threshold)
  devices_data={}
  for r in rows:

 ...
 try:
  ...
  async with httpx.AsyncClient(timeout=10.0) as client:
   ...
   if isinstance(weather_response,Exception):print(f"[{datetime.datetime.now()}] ERROR: Weather API exception: {weather_response}")
    ...
    else:print(f"[{datetime.datetime.now()}] ERROR: Weather API status {weather_response.status_code}")
...
def transform_device_data(received_data):
    ...
    network_active = None
    if received_data.get('bssid') and received_data.get('bssid') not in ['0', '', 'error']:
        network_active = 'Wi-Fi'
    elif received_data.get('nt'):
        network_active = received_data.get('nt')
    
    last_refresh_time = None
    if 'received_at' in received_data:
        try:
            ...
            if isinstance(received_time, str):
                ...
                last_refresh_time = received_time.strftime("%Y-%m-%d %H:%M:%S UTC")
        except Exception as e:
            print(f"Error formatting last_refresh_time: {e}")
    
    if not last_refresh_time:
        last_refresh_time = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    ...
    result = {
        ...
        'cell_signal_strength': f"{safe_int(received_data.get('rssi'))} dBm" if received_data.get('rssi') is not None and received_data.get('rssi') not in [0, 1] else None,
...
def transform_device_data(received_data):
    ...
    # Get network type from proper field
    network_active = None
    if received_data.get('bssid') and received_data.get('bssid') not in ['0', '', 'error']:
        network_active = 'Wi-Fi'
    elif received_data.get('nt'):
    ...
    # Format last refresh time in UTC
    last_refresh_time = None
    ...
    # If no received_at, use current time
    if not last_refresh_time:
        last_refresh_time = datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")

    # Create the final data structure with proper handling of values
    result = {
        'device_id': received_data.get('id'),
        'device_name': received_data.get('n'),
        'battery_percent': f"{safe_int(received_data.get('perc'))}%" if received_data.get('perc') is not None else None,

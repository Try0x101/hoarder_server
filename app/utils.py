    ...
    try:
        ...
        if tz_str.startswith("UTC"):
            if tz_str == "UTC":
                ...
                if ":" in offset_str:
                    hours, minutes = offset_str.replace("+", "").replace("-", "").split(":")
                    offset = datetime.timedelta(hours=sign * int(hours), minutes=int(minutes))
                else:
                    offset = datetime.timedelta(hours=sign * int(offset_str.replace("+", "").replace("-", "")))
        ...
        print(f"Error calculating weather data age: {e}")
        return None
...
def transform_device_data(received_data):
    ...
    weather_fetch_data_age = None
    if weather_fetch_formatted:
        weather_fetch_data_age = calculate_weather_data_age(weather_fetch_formatted)


    # Format last refresh time in the device's timezone
    ...
    if 'received_at' in received_data and received_data.get('received_at'):
        try:
            ...
            if isinstance(received_time, datetime.datetime):
                ...
                # Convert to device timezone if available
                if location_tz:
    ...
    # Calculate age of last refresh time
    ...
    return {
        ...
        'last_refresh_time_data_age': last_refresh_time_data_age
    }

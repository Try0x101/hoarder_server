...
async def get_history(
    ...
    limit:int=Query(1000,description="Maximum number of records to return per page"),
    ...
    try:
        ...
        try:
            if not device_id:
                ...
                SELECT device_id,MAX(received_at) as last_active,COUNT(*) as record_count
                FROM device_data
                WHERE received_at >= $1
                GROUP BY device_id
                ORDER BY last_active DESC
                """
                devices=await conn.fetch(active_devices_query,cutoff_date)
                ...
                for device in devices:
                    device_links.append({
                        ...
                        "links":{
                            "history":f"/data/history?device_id={device['device_id']}",
                            "gaps":f"/data/gaps?device_id={device['device_id']}",
                            "summary":f"/data/summary?device_id={device['device_id']}"
                        }
                    })
                return {
                    ...
                    "notice":"To view data for a specific device, click on the history link or add '?device_id=DEVICE_ID' to the URL"
                }
            device_exists=await conn.fetchval("SELECT EXISTS(SELECT 1 FROM latest_device_states WHERE device_id = $1)",device_id)
            if not device_exists:
                return JSONResponse(status_code=404,content={"error":f"Device with ID '{device_id}' not found"})
            ...
            LIMIT $3 OFFSET $4
            """
            rows=await conn.fetch(query,device_id,cutoff_date,limit,offset)
            results=[]
            for row in rows:
                try:
                    payload=row["payload"]
            ...
            response={
                ...
                "pagination_help":{
                    "instructions":"To navigate through pages, use the offset parameter in the URL.",
                    "examples":{
                        "first_page":f"/data/history?device_id={device_id}&offset=0",
                        "page_2":f"/data/history?device_id={device_id}&offset=1000",
                        "page_3":f"/data/history?device_id={device_id}&offset=2000"
                    },
                    "tip":"Each page shows up to 1000 records. To see the next 1000 records, increase the offset by 1000."
                },
                "links":{
                    "back_to_devices":f"/data/history",
...
async def get_data_gaps(device_id:str=Query(...,description="Device ID to analyze")):
    try:
        ...
        try:
            query="""
            WITH time_series AS (
                SELECT device_id,received_at,LEAD(received_at) OVER (PARTITION BY device_id ORDER BY received_at) as next_timestamp
                FROM device_data 
                WHERE device_id = $1 AND received_at >= $2
            )
            SELECT device_id,received_at as gap_start,next_timestamp as gap_end,
            EXTRACT(EPOCH FROM (next_timestamp - received_at))/60 as gap_minutes
            FROM time_series
            WHERE EXTRACT(EPOCH FROM (next_timestamp - received_at))/60 > 15
            ...
            EXTRACT(EPOCH FROM (MAX(received_at) - MIN(received_at)))/3600 as hours_span
            FROM device_data
            WHERE device_id = $1 AND received_at >= $2
            """
            ...
            return {
                ...
                "statistics":{
                    ...
                    "average_record_interval_minutes":(stats["hours_span"]*60/stats["record_count"]) if stats["record_count"]>0 else None
                },
                "gaps":[
                    {
    ...
    try:
        ...
        try:
            ...
            WITH hourly_stats AS (
                SELECT device_id,DATE_TRUNC('hour', received_at) as hour,COUNT(*) as records_per_hour
                FROM device_data
                WHERE device_id = $1 AND received_at >= $2
                GROUP BY device_id, hour
                ORDER BY hour
            )
            SELECT AVG(records_per_hour) as avg_records_per_hour,
            MAX(records_per_hour) as max_records_per_hour,
            MIN(records_per_hour) as min_records_per_hour,
            ...
            WITH position_data AS (
                SELECT (payload->>'lat')::float as lat,(payload->>'lon')::float as lon
                FROM device_data
                WHERE device_id = $1 AND received_at >= $2
                AND payload->>'lat' IS NOT NULL AND payload->>'lon' IS NOT NULL
                AND (payload->>'lat')::float BETWEEN -90 AND 90
                AND (payload->>'lon')::float BETWEEN -180 AND 180
            )
            SELECT COUNT(*) as position_count,AVG(lat) as avg_lat,
            AVG(lon) as avg_lon,MIN(lat) as min_lat,MAX(lat) as max_lat,
            ...
            return {
                ...
                "activity_statistics":{
                    ...
                    "activity_percentage":(stats["active_hours"]/total_hours)*100 if total_hours>0 else 0,
                    "avg_records_per_hour":stats["avg_records_per_hour"],
                    "max_records_per_hour":stats["max_records_per_hour"],
                    "min_records_per_hour":stats["min_records_per_hour"]
                },
                "position_statistics":{
                    "has_position_data":position_stats["position_count"]>0,
                    "position_count":position_stats["position_count"],
                    "center_position":{
                        "lat":position_stats["avg_lat"],
                        "lon":position_stats["avg_lon"]
                    } if position_stats["position_count"]>0 else None,
                    "bounds":{
                        "min_lat":position_stats["min_lat"],
                        "max_lat":position_stats["max_lat"],
                        "min_lon":position_stats["min_lon"],
                        "max_lon":position_stats["max_lon"]
                    } if position_stats["position_count"]>0 else None
                }
            }
        finally:await conn.close()

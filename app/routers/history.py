...
async def get_history(device_id:str=Query(None,description="Device ID to get history for (optional)"),limit:int=Query(1000,description="Maximum number of records to return per page"),offset:int=Query(0,description="Number of records to skip for pagination")):
 try:
  ...
  try:
   if not device_id:
    active_devices_query="SELECT device_id,MAX(received_at) as last_active,COUNT(*) as record_count FROM device_data WHERE received_at>=$1 GROUP BY device_id ORDER BY last_active DESC"
    ...
    for device in devices:
     device_links.append({
      ...
      "links":{"history":f"/data/history?device_id={device['device_id']}","gaps":f"/data/gaps?device_id={device['device_id']}","summary":f"/data/summary?device_id={device['device_id']}"}
     })
   ...
   query="SELECT id,device_id,payload,received_at FROM device_data WHERE device_id=$1 AND received_at>=$2 ORDER BY received_at DESC LIMIT $3 OFFSET $4"
   ...
   response={
    ...
    "pagination":{"current_page":current_page,"total_pages":total_pages,"limit":limit,"offset":offset},
    "pagination_help":{"instructions":"To navigate through pages, use the offset parameter in the URL.","examples":{"first_page":f"/data/history?device_id={device_id}&offset=0","page_2":f"/data/history?device_id={device_id}&offset=1000","page_3":f"/data/history?device_id={device_id}&offset=2000"},"tip":"Each page shows up to 1000 records. To see the next 1000 records, increase the offset by 1000."},
    "links":{"back_to_devices":f"/data/history","gaps":f"/data/gaps?device_id={device_id}","summary":f"/data/summary?device_id={device_id}"}
   }
  ...
  try:
   ...
   return{
    ...
    "position_statistics":{
     ...
     "center_position":{"lat":position_stats["avg_lat"],"lon":position_stats["avg_lon"]} if position_stats["position_count"]>0 else None,
     "bounds":{"min_lat":position_stats["min_lat"],"max_lat":position_stats["max_lat"],"min_lon":position_stats["min_lon"],"max_lon":position_stats["max_lon"]} if position_stats["position_count"]>0 else None
    }
   }
  finally:await conn.close()
 except Exception as e:

...
async def export_database(device_id:Optional[str]=None,background_tasks:BackgroundTasks=None):
    try:
        ...
        with open(status_file,"w") as f:
            json.dump({"status":"started","timestamp":datetime.datetime.now().isoformat()},f)
...
async def export_status(export_id:str):
    ...
    if not os.path.exists(status_file):
        return{"status":"not_found","message":"Export ID not found"}
...
async def import_status(import_id:str):
    ...
    if not os.path.exists(status_file):
        return{"status":"not_found","message":"Import ID not found"}
...
async def perform_export(export_file:str,status_file:str,device_id:Optional[str]=None):
    ...
    try:
        ...
        update_status(status_file,{
            ...
            "records_exported":len(device_data),
...
async def perform_import(import_file:str,status_file:str,merge:bool=True):
    ...
    try:
        ...
        update_status(status_file,{
            "status":"in_progress",
            "message":f"Processing {total_records} records",
            ...
            try:
                if merge:
                    ...
                    if not exists:
                        ...
                        await conn.execute("INSERT INTO device_data (id,device_id,payload,received_at) VALUES ($1,$2,$3,$4)",
                            row["id"],row["device_id"],row["payload"],received_at)
                else:
                    received_at=datetime.datetime.fromisoformat(row["received_at"]) if isinstance(row["received_at"],str) else row["received_at"]
                    await conn.execute("INSERT INTO device_data (id,device_id,payload,received_at) VALUES ($1,$2,$3,$4)",
                        row["id"],row["device_id"],row["payload"],received_at)
            except Exception as e:
                print(f"[{datetime.datetime.now()}] Error importing device_data row: {str(e)}")
            ...
            try:
                ...
                if merge:
                    ...
                    ON CONFLICT (device_id) DO UPDATE SET
                        payload=CASE WHEN latest_device_states.received_at<$3 THEN $2 ELSE latest_device_states.payload END,
                        received_at=GREATEST(latest_device_states.received_at,$3)
                    """,row["device_id"],row["payload"],received_at)
                else:
                    await conn.execute("INSERT INTO latest_device_states (device_id,payload,received_at) VALUES ($1,$2,$3)",
                        row["device_id"],row["payload"],received_at)
            except Exception as e:
                print(f"[{datetime.datetime.now()}] Error importing latest_device_states row: {str(e)}")
            ...
            try:
                if merge:
                    ...
                    if not exists:
                        ...
                        received_at=datetime.datetime.fromisoformat(row["received_at"]) if isinstance(row["received_at"],str) else row["received_at"]
                        await conn.execute("""
                        INSERT INTO timestamped_data (id,device_id,payload,data_timestamp,received_at,data_type,is_offline,batch_id)
                        VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
                        """,row["id"],row["device_id"],row["payload"],data_timestamp,received_at,
                            row.get("data_type","telemetry"),row.get("is_offline",False),row.get("batch_id"))
                else:
                    data_timestamp=datetime.datetime.fromisoformat(row["data_timestamp"]) if isinstance(row["data_timestamp"],str) else row["data_timestamp"]
                    received_at=datetime.datetime.fromisoformat(row["received_at"]) if isinstance(row["received_at"],str) else row["received_at"]
                    await conn.execute("""
                ...
                print(f"[{datetime.datetime.now()}] Error importing timestamped_data row: {str(e)}")
        ...
        update_status(status_file,{
            "status":"completed",
            "message":f"Import completed. {processed} records imported.",
...
def update_status(status_file:str,update:dict):
    try:
        ...
        print(f"[{datetime.datetime.now()}] Error updating status: {str(e)}")
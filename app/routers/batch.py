...
async def receive_batch_data(request:Request,background_tasks:BackgroundTasks):
    try:
        ...
        if not isinstance(batch_data,list):
            raise HTTPException(status_code=400,detail="Expected a JSON array of telemetry data objects")
        processed_count=0
        ...
        for item in batch_data:
            ...
            if 'lat' in item and 'lon' in item and item['lat'] and item['lon']:
                ...
                if any(k.startswith('weather_') for k in item.keys()):
                    weather_enriched_count+=1
        ...
        return {
            ...
            "weather_enriched":weather_enriched_count,
